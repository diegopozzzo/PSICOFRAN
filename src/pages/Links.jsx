...
function LinkCard({ href, interno, icon: IconCmp, titulo, desc, destacado, delay }) {
  const inner = (
    <span
      className={`group flex w-full items-center gap-4 rounded-[calc(2rem-0.375rem)] p-5 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
        destacado ? 'bg-espresso-900 text-cream' : 'bg-white text-espresso-900'
      }`}
    >
      <span
        className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ring-1 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:-translate-y-0.5 group-hover:scale-105 ${
...
export default function Links() {
  return (
    <main className="relative flex min-h-[100dvh] items-center justify-center overflow-x-clip px-4 py-16">
      ...
      <div className="relative w-full max-w-md">
        ...
        <Reveal className="text-center">
          <span className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-blush-100 via-cream to-sage-100 font-serif text-3xl text-blush-500 ring-1 ring-espresso-900/[0.07] shadow-[0_24px_60px_-30px_rgba(54,34,39,0.4)]">
            FL
          </span>
          <h1 className="mt-5 font-serif text-3xl tracking-tight text-espresso-900">
            Francis Landeo
          </h1>
        ...
        <Reveal delay={600} className="mt-9 text-center">
          <Link
            to="/"
